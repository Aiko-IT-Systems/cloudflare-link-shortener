...
const linkTimestampFields = ["createdAt", "metadataFetchedAt", "expiresAt", "disabledAt"] as const;
const writableLinkTimestampFields = ["metadataFetchedAt", "expiresAt", "disabledAt"] as const;
const apiTimestampFields = new Set(["createdAt", "metadataFetchedAt", "expiresAt", "disabledAt", "deletedAt", "revokedAt"]);
...
export function canonicalizeWritableLinkTimestamps<T extends LinkRecord>(record: T): T {
	return canonicalizeTimestampFields(record, writableLinkTimestampFields);
}

function canonicalizeTimestampFields<T extends LinkRecord>(record: T, fields: readonly (keyof LinkRecord)[]): T {
	const result = { ...record };
...
export function patchApiTimestamps<T>(value: T): T {
	...
	const record = Object.fromEntries(Object.entries(value as Record<string, unknown>)
		.map(([key, child]) => [key, apiTimestampFields.has(key) && typeof child === "string" ? canonicalTimestamp(child) : patchApiTimestamps(child)]));
	return record as T;
}
