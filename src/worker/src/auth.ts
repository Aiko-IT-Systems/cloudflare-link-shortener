...
export async function authenticate(env: Env, token: string): Promise<AuthPrincipal | undefined> {
	...
	if (!record || record.revokedAt || !timingSafeEqual(await sha256(token), record.digest)) return undefined;
...
export async function requireApiKey(c: Context<{ Bindings: Env; Variables: { principal: AuthPrincipal } }>, next: Next): Promise<Response | void> {
	const header = c.req.header("Authorization");
	if (!header?.startsWith("Bearer ")) return jsonError("Missing bearer token.", "missing_auth", 401);
	const token = header.slice("Bearer ".length).trim();
	if (!token) return jsonError("Missing bearer token.", "missing_auth", 401);
	const principal = await authenticate(c.env, token);
	if (!principal) return jsonError("Invalid bearer token.", "invalid_auth", 401);
	c.set("principal", principal);
	return next();
}

export function requireAdmin(principal: AuthPrincipal): Response | undefined {
	return principal.kind === "admin" ? undefined : jsonError("Administrator access is required.", "admin_required", 403);
}
