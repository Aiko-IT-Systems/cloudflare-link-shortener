import { bootstrapAdminAccount, createLink, disableLink, getAccountByDiscordUserId, getLink, listLinks, listOwnedLinks, refreshLinkMetadata, updateLink } from "./store";
...
type Interaction = {
	...
	data?: { name?: string; custom_id?: string; target_id?: string; options?: Array<{ name: string; value?: string }>; resolved?: { messages?: Record<string, { content?: string }> }; components?: unknown[] };
	member?: { user?: DiscordUser };
	user?: DiscordUser;
};
type CreateSession = { type: "create"; user: DiscordUser; accountId: string; urls: string[]; index: number };
type ManageSession = { type: "manage"; userId: string; accountId: string; admin: boolean; cursor?: string; previous: string[] };
type Session = CreateSession | ManageSession;

function sessionKey(id: string): string { return `discord-session:${id}`; }
function sessionId(): string { return crypto.randomUUID().replaceAll("-", ""); }
function response(body: unknown, status = 200): Response { return Response.json(body, { status }); }
function ephemeral(content: string): Response { return response({ type: 4, data: { content, flags: EPHEMERAL, allowed_mentions: { parse: [] } } }); }
function v2(components: unknown[]): Response { return response({ type: 4, data: { flags: EPHEMERAL | COMPONENTS_V2, components, allowed_mentions: { parse: [] } } }); }
function text(content: string): Record<string, unknown> { return { type: 10, content }; }
function button(customId: string, label: string, style = 2, disabled = false): Record<string, unknown> { return { type: 2, custom_id: customId, label, style, disabled }; }
function linkButton(url: string): Record<string, unknown> { return { type: 2, style: 5, label: "Open link", url }; }
...
function modal(session: CreateSession, id: string, currentTitle?: string, customId = `short:create:${id}`): Response {
	...
	return response({
		...
		data: {
			custom_id: customId,
			title: editing ? "Edit short link" : session.urls.length > 1 ? `Shorten ${session.index + 1}/${session.urls.length}` : "Create short link",
			components: [
				text(`Destination\n${url}`),
...
function adminSetupModal(): Response {
	return response({
		...
		data: {
			...
			components: [label("Public author name", "creatorName", "Shown as the author of your new links")]
		}
	});
}

function label(labelText: string, customId: string, placeholder: string, value?: string, password = false): Record<string, unknown> {
	return { type: 18, label: labelText, component: { type: 4, custom_id: customId, style: 1, required: false, placeholder, ...(value ? { value } : {}), ...(password ? { style: 1 } : {}) } };
}

async function saveSession(env: Env, id: string, session: Session): Promise<void> {
	await env.LINKS.put(sessionKey(id), JSON.stringify(session), { expirationTtl: SESSION_TTL_SECONDS });
}
async function loadSession(env: Env, id: string): Promise<Session | null> { return env.LINKS.get<Session>(sessionKey(id), "json"); }
async function clearSession(env: Env, id: string): Promise<void> { await env.LINKS.delete(sessionKey(id)); }

function userOf(interaction: Interaction): DiscordUser | undefined { return interaction.member?.user ?? interaction.user; }
...
function extractUrls(content: string): string[] {
	...
	for (const candidate of content.match(/https:\/\/[^\s<>()]+/g) ?? []) {
		...
		} catch { /* Ignore malformed message text. */ }
	}
	return [...seen];
}
...
function submittedValues(components: unknown[]): Record<string, string> {
	...
	const visit = (item: unknown): void => {
		...
		const record = item as { custom_id?: unknown; value?: unknown; components?: unknown[]; component?: unknown };
		if (typeof record.custom_id === "string" && typeof record.value === "string") values[record.custom_id] = record.value.trim();
...
async function verifyDiscordRequest(request: Request, env: Env): Promise<Uint8Array | undefined> {
	...
	const publicKey = await crypto.subtle.importKey("raw", hex(keyHex), { name: "Ed25519" }, false, ["verify"]);
	return await crypto.subtle.verify({ name: "Ed25519" }, publicKey, hex(signature), data) ? raw : undefined;
}

function isDiscordAdmin(env: Env, user: DiscordUser): boolean { return Boolean(env.DISCORD_ADMIN_USER_ID) && env.DISCORD_ADMIN_USER_ID === user.id; }

function canManage(account: AccountRecord, record: LinkRecord, admin: boolean): boolean {
	return admin || (record.owner?.kind === "account" && record.owner.id === account.id);
}

async function listManageLinks(env: Env, session: ManageSession, cursor = session.cursor): Promise<LinkPage> {
	const loadPage = (nextCursor?: string): Promise<LinkPage> => session.admin
		? listLinks(env, nextCursor, 1)
		: listOwnedLinks(env, { kind: "account", id: session.accountId }, nextCursor, 1);
	...
	while (true) {
		...
		if (record && !record.disabledAt) {
			if (items.length === MANAGE_PAGE_SIZE) return { items, cursor: nextCursor };
			items.push(record);
		}
...
async function renderManage(env: Env, origin: string, sessionIdValue: string, session: ManageSession): Promise<Response> {
	...
	const nav = row(
		button(`short:manage-prev:${sessionIdValue}`, "Previous", 2, session.previous.length === 0),
		button(`short:manage-next:${sessionIdValue}`, "Next", 2, !page.cursor)
	);
	return v2([
		container(text(session.admin ? "## All links (admin)" : "## Your linked-account links")),
		...cards,
		container(nav)
	]);
}
...
async function handleCommand(interaction: Interaction, env: Env, origin: string, user: DiscordUser, account: AccountRecord): Promise<Response> {
	...
	if (name === "shorten") {
		...
		if (!url || !extractUrls(url).length || extractUrls(url)[0] !== url) return ephemeral("Please provide one valid HTTPS URL.");
		const id = sessionId();
		const session: CreateSession = { type: "create", user, accountId: account.id, urls: [url], index: 0 };
		await saveSession(env, id, session);
		return modal(session, id);
	}
	if (name === "shorten link") {
		const target = interaction.data?.target_id;
		const content = target ? interaction.data?.resolved?.messages?.[target]?.content ?? "" : "";
		const urls = extractUrls(content);
		if (!urls.length) return ephemeral("That message does not contain an HTTPS link I can shorten.");
		const id = sessionId();
		const session: CreateSession = { type: "create", user, accountId: account.id, urls, index: 0 };
		await saveSession(env, id, session);
		return modal(session, id);
	}
	if (name === "manage") {
...
async function handleModal(interaction: Interaction, env: Env, origin: string, user: DiscordUser, account: AccountRecord, customId: string): Promise<Response> {
	...
	if (action === "edit-submit" && id) {
		...
		if (!record || !canManage(account, record, isDiscordAdmin(env, user))) return ephemeral("Link not found.");
	...
	if (!session || session.type !== "create" || session.user.id !== user.id || session.accountId !== account.id) return ephemeral("This form has expired or belongs to another user.");
	const values = submittedValues(interaction.data?.components ?? []);
	const payload = createLinkSchema.safeParse({ destinationUrl: session.urls[session.index], slug: values.slug || undefined, title: values.title || undefined, password: values.password || undefined, expiresAt: values.expiresAt || undefined });
	if (!payload.success) return ephemeral("The link details were invalid. Please start again and check the expiry format.");
	const metadata = await fetchTargetMetadata(payload.data.destinationUrl);
	const created = await createLink(env, { ...payload.data, creator: user.username, owner: { kind: "account", id: account.id }, embedTitle: metadata.embedTitle, embedDescription: metadata.embedDescription, embedImageUrl: metadata.embedImageUrl, embedSiteName: metadata.embedSiteName, metadataFetchedAt: metadata.metadataFetchedAt });
	if (created === "duplicate") return ephemeral("That custom slug is already in use.");
...
async function handleAdminSetup(interaction: Interaction, env: Env, user: DiscordUser): Promise<Response> {
	if (!isDiscordAdmin(env, user)) return ephemeral("Administrator access is required.");
	const creatorName = submittedValues(interaction.data?.components ?? []).creatorName;
	if (!creatorName || creatorName.length > 80) return ephemeral("Please provide a public author name up to 80 characters long.");
	try {
		const account = await bootstrapAdminAccount(env, user.id, creatorName);
		return ephemeral(`Administrator profile ${account.id} is ready. Existing links now belong to it; run /manage again.`);
	} catch {
		return ephemeral("Could not set up the administrator profile. Please contact an administrator.");
	}
}
...
async function handleComponent(interaction: Interaction, env: Env, origin: string, user: DiscordUser, account: AccountRecord, customId: string): Promise<Response> {
	...
	if (action === "next" || action === "abort") {
		...
		if (!session || session.type !== "create" || session.user.id !== user.id || session.accountId !== account.id) return ephemeral("This batch has expired or belongs to another user.");
		if (action === "abort") { await clearSession(env, value); return v2([container(text("## Batch stopped\nLinks already created remain available."))]); }
	...
	if (!record || !canManage(account, record, admin)) return ephemeral("Link not found.");
	if (action === "edit") {
		const id = sessionId();
...
export async function handleDiscordInteraction(request: Request, env: Env, origin = new URL(request.url).origin): Promise<Response> {
	...
	try { interaction = JSON.parse(new TextDecoder().decode(raw)) as Interaction; } catch { return new Response("Invalid JSON.", { status: 400 }); }
	if (interaction.application_id && env.DISCORD_APPLICATION_ID && interaction.application_id !== env.DISCORD_APPLICATION_ID) return new Response("Unknown application.", { status: 401 });
	...
	if (!account) {
		if (interaction.type === 2 && interaction.data?.name?.toLowerCase() === "manage" && isDiscordAdmin(env, user)) return adminSetupModal();
